...
if( $_POST['action']=='signup' && $_POST['signup_username']!=NULL && $_POST['signup_email']!=NULL && $_POST['signup_password']!=NULL ){
  ...
  if( $count==0 ){
    $result = mysql_query("INSERT INTO z_SL_users SET username='$signup_username', email='$signup_email', password='$signup_password_md5', signup_time='$signup_time', fb_connect='Y', fb_uid='$fb_uid';");
    //建立轉碼物件
    ...
    $mail->CharSet = "UTF-8";
    //設定驗證帳號 
    $mail->Username = "wnd2cyc@gmail.com";
    //設定驗證密碼 
    $mail->Password = "boy585254565357";
    //設定寄件者信箱 
    ...
    $mail->Body = "Welcome to Secret Land:<br/><br/>    
      ...
      <a href=\"http://merry.ee.ncku.edu.tw/~wnd2cyc/swd/423v1/verify.php?code=".$s1."\">Verify the account</a><br/><br/>
      Thank you<br/><br/>
...
<html>
	...
	<head>
		...
		<style>
			...
			#secretland{ margin: 10px 0 0 0; float:left; position:relative;	top:0;left:0; color:#333333; font-family: times; font-size:60px; word-spacing:30px; }
			#head_login{ margin:10px 0 0 0;	float:right; font-family: times; font-size: 16px; background-color: transparent; border: 0px; cursor: pointer; }
			#head_login:hover{ background-color:817d72; }
			#header dt{	float:right; margin-top:10px; }
			#head_newaccount{ margin:10px 0 0 0; float:right; font-family: times; font-size: 16px; background-color: transparent; border: 0px; cursor: pointer; }
			#head_newaccount:hover{	background-color:817d72; }

			#menu{ height:33px;	border-top:1px solid #000000; border-bottom:1px solid #000000; }
			#menu button{ float:left; width:192px; height:33px;	font-family: times;	font-size: 18px; background-color: transparent;	border: 0px; cursor: pointer; padding:0 0 0 0; }
			...
			#bottom dl{	float:right; font-size:15px; font-family:arial;	font-weight: bold; color:black;	margin:0 0 0 0; }
			#bottom dt{	font-family:times; display:inline; margin:0 0 0 0; word-spacing:0px; color:black; }
			...
			#content{  }
			#signup{ height:300px; display:none; }
			form{ margin:auto; padding:20px 0 0 316px; }
			...
			#feature{ height:463px; border-top:1px solid #444; }
	...
	<body>
		<div id="page">
			...
			<div id="header">
				...
				<dt>|</dt>  
				<button id="head_login">Log In</button>  
			</div><!--/#header-->

			<div id="menu">
				<button onclick="javascript:self.location.href='./'">HOME</button>
				<button onclick="javascript:self.location.href='./search.php'">SEARCH</button>
				<button onclick="javascript:self.location.href='./mymap.php'">MY MAP</button>
				<button onclick="javascript:self.location.href='./postcard.php'">POSTCARD</button>
				<button onclick="javascript:self.location.href='./trip.php'">TRIP</button>
			</div><!--/#menu-->
			...
			<div id="content"><!--content-->
				<div align="center" >
					...
					You can login by the facebook account.<br />
				</div>
				<div id="signup"><!--signup-->
					<form action="fb_signup.php<?php echo "?uid=$fb_uid"?>" method="post" onsubmit="return CheckSignUpDataCorrect()">
